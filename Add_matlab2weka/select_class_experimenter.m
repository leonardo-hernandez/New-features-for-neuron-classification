function select_class_experimenter(ft_train_weka,groups,file,percent)
...
for l = 1 : size(groups,2)
    ...
    if rseed
        NClass = 100; % Numero de veces que se clasificara por cada selecciï¿½n
    else
        NClass = 1;
    end
    ...
    second = ['Evaluador:, '  evaluador ',' 'Búsqueda:, ' search ',' 'Clasificador:, ' classif];
%     tree = 'ATTR, %ICC, NTI, ICC, ICI, ROC, F-STAT, PROM, MIN,  MAX, SD, MAX-MIN, FEAT1, FEAT2, FEAT3, FEAT4, FEAT5';
    ...
    if size(featName,2) <= diez_percent;
        for h = 1:NClass
            [Correctly_Classified_Instances, Total_Number_of_Instances, Correct_Intances, Incorrect_Intances, ROC_Area, F_Measure] = wekaClassification(train_weka, classifgroup, classif, rseed, nfold);
...
for g = 1:size(featlist,2);
    ...
    train_weka = ft_train_weka;
            ...
            disp([   'Evaluador: ' evaluador '  Búsqueda: ' search '  Clasificador: ' classif]) 
